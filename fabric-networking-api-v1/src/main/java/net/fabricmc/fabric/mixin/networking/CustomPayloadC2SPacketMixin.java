...
public class CustomPayloadC2SPacketMixin implements SplittablePacket {
	@Shadow
	@Final
	...
	public void fabric_split(int id, NetworkState<?> state, ChannelHandlerContext channelHandlerContext, EncoderHandler<?> encoder, Packet<?> packet, Consumer<Packet<?>> consumer) throws Exception {
		PayloadTypeRegistryImpl<?> registry = state.id() == NetworkPhase.CONFIGURATION ? PayloadTypeRegistryImpl.CONFIGURATION_C2S : PayloadTypeRegistryImpl.PLAY_C2S;
		...
		FabricPacketSplitter.customFabricSplit(id, channelHandlerContext, encoder, packet, CustomPayloadC2SPacket::new, consumer, FabricPacketSplitter.SAFE_C2S_SPLIT_SIZE, size);
	}
}
